...
const Home = () => {
  ...
  const [showSubscription, setShowSubscription] = useState(false); // ✅ MODAL STATE
  ...
  const handleChangeScreen = (screenName, data = {}) => {
    ...
    if (!isSubscribed && screenName !== "admin") {
      setShowSubscription(true); // ✅ OPEN MODAL
      return;
    }
            ...
            <div className="home-game-grid">
              {games.map((game, index) => (
                <GameCard
                  ...
                  {...game}
                  delay={index * 0.1}
                  onPlay={() => handleChangeScreen(game.name)}
                />
              ))}
            </div>
          </div>
